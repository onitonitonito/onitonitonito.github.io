...
<!-- Let's -->
'<span id="start-ch">Let's</span>' have a look at [[Cookie Bike](https://bit.ly/2WSi97e)]
...
<!-- LED -->
'<span id="start-ch">LED,</span>'
lights were supplied its power from the hub generator which is mounted in the front wheel hub.

<a href="http://203.234.189.78/wp-content/uploads/2018/06/20180531_123837.jpg">
<img width="500" src="http://203.234.189.78/wp-content/uploads/2018/06/20180531_123837.jpg">
</a>

<!-- Automated -->
'<span id="start-ch">Automated,</span>' lock unlocks automatically just after QR code recognition, which is applied by a smartphone camera. and the fare only charged during the lock is unlocked. if you shut the lock, service will be terminated and will get the receipt by text.
...
<!--the  -->
'<span id="start-ch">the </span>' 3-level grip gear on the right handle.
...
<!--the  -->
'<span id="start-ch">the </span>' inner hub gear set, which deprived derailer that mainly causes a lot of problems.
...
<!--the  -->
'<span id="start-ch">the </span>' front hub generator and belt breaking system.
...
<!--the  -->
'<span id="start-ch">the </span>' telescopic seat post and adjust-lever give comfort height.
...
<!--the  -->
'<span id="start-ch">the </span>' self-return spoke gives wheel position straight when you are parking bike.
...
<!--0.2$  -->
'<span id="start-ch">0.2$ </span>' per 20 min. will be a good deal. 5$ deposit and the extra prepaid charge will be decreased according to the service period.
...
<!--See  -->
'<span id="start-ch">See</span>' well-arranged front wheels due to the self-return spoke with spring.
...
<!--Bike id -->
'<span id="start-ch">Bike id</span>', service time (start and stop), used cash and the balance (remains). I charged prepay about $10. but it is free promotion period until August, 1st. FREE! FREE! FREE!
...
<!--You -->
'<span id="start-ch">You</span>'can find an available bike around you in real-time,&nbsp; (F) means an available individual bike and the yellow icon is a station, which you can find a number of bikes to use.
...
<!--It -->
'<span id="start-ch">It</span>' shows network error message sometimes but it's no big deal with service, only a little border.
